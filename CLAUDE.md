...
Before pasting a component or rolling your own, check `components/ui/` (Button, Alert, Card, Dialog, Badge, Input, Toast, etc.). Use those. Only build new when nothing fits — and put it in `components/ui/` so it's reusable.

### Responsive priority: iPad → mobile → desktop
Most students browse on iPad, then phone. Desktop is last. Design and test in that order. Default breakpoints should look right on iPad first.
...
- CSS: Tailwind v4. Bootstrap 5 framework removed; a shrinking Bootstrap compatibility layer remains in `styles/tailwind.css` for the remaining unmigrated pages. Don't grow it — migrate to Tailwind utilities instead.
...
- Responsive order: iPad first, then mobile, then desktop (see Hard Constraints)
