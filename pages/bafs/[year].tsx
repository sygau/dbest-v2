...
function getPaperDisplayInfo(paperId: string, year: string): PaperData | null {
  const bafsPaperTypeMapEng: Record<string, string> = {
    'p1': 'Paper 1',
    'p2a': 'Paper 2A',
    'p2b': 'Paper 2B',
  ...
  const bafsPaperTypeMapChi: Record<string, string> = {
    'p1': '卷一',
    'p2a': '卷二甲',
    'p2b': '卷二乙',
    'ans': '參考答案',
  ...
  const displayType = language === 'chi' 
    ? bafsPaperTypeMapChi[paperType] || paperType.toUpperCase()
    : bafsPaperTypeMapEng[paperType] || paperType.toUpperCase();
...
export default function BAFSYearPage({ subject, year, papers, availableFiles }: YearPageProps) {
  // Use the clean single function approach
  const meta = generateYearMeta('bafs', year);
  const lastUpdated = getSubjectYearSlugLastUpdated('bafs');
  ...
  return (
    <>
      <Head>
        ...
        <meta property="og:url" content={`https://dse.best/bafs/${year}`} />
      ...
      <div className="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
        <div className="breadcrumb-title pe-3">BAFS</div>
        <div className="ps-3">
          ...
          {chinesePapers.length > 0 && (
            <div className="mb-5">
              ...
              <div className="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-4">
                {chinesePapers
                  .sort((a, b) => {
                    // Sort order: Paper 1, Paper 2A, Paper 2B, then Answers, then Performance
                    const order: Record<string, number> = { '卷一': 1, '卷二甲': 2, '卷二乙': 3, '參考答案': 4, '考生表現報告': 5 };
            ...
            <div className="mb-5">
              ...
              <div className="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-4">
                {englishPapers
                  .sort((a, b) => {
                    // Sort order: Paper 1, Paper 2A, Paper 2B, then Answers, then Performance
                    const order: Record<string, number> = { 'Paper 1': 1, 'Paper 2A': 2, 'Paper 2B': 3, 'Answers / Marking Scheme': 4, 'Performance Report': 5 };
                    return (order[a.paperType] || 999) - (order[b.paperType] || 999);
                  })
                  .map((paper) => (
                  <div key={paper.paperId} className="col">
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  ...
  try {
    // Read bafs config file
    const configPath = path.join(process.cwd(), 'public', 'config', 'bafs.json');
    ...
    console.error(`Error loading bafs ${year}:`, error);
    return {
      notFound: true
    };
  }
