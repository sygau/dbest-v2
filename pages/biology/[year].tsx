...
function getPaperDisplayInfo(paperId: string, year: string): PaperData | null {
  const biologyPaperTypeMapEng: Record<string, string> = {
    'p1': 'Paper 1',
    'p2': 'Paper 2',
  ...
  const biologyPaperTypeMapChi: Record<string, string> = {
    'p1': '卷一',
    'p2': '卷二',
    'ans': '參考答案',
    'per': '考生表現報告'
  ...
  const displayType = language === 'chi'
    ? biologyPaperTypeMapChi[paperType] || paperType.toUpperCase()
    : biologyPaperTypeMapEng[paperType] || paperType.toUpperCase();
...
export default function BiologyYearPage({ subject, year, papers, availableFiles }: YearPageProps) {
  // Use the clean single function approach
  const meta = generateYearMeta('biology', year);
  const lastUpdated = getSubjectYearSlugLastUpdated('biology');
  ...
  return (
    <>
      <Head>
        ...
        <meta property="og:url" content={`https://dse.best/biology/${year}`} />
        ...
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              "url": `https://dse.best/biology/${year}`,
              "mainEntity": {
      ...
      <div className="card rounded-4" style={{ height: "auto" }}>
        <div className="card-body">
          ...
          <div className="text-center mt-5 mb-5">
            <h3>Need More Biology Papers?</h3>
            ...
            >
              <span>View All Biology Papers (2012-2024)</span>
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  ...
  try {
    // Read biology config file
    const configPath = path.join(process.cwd(), 'public', 'config', 'biology.json');
    ...
    console.error(`Error loading biology ${year}:`, error);
    return {
      notFound: true
    };
  }
