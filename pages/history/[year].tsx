...
function getPaperDisplayInfo(paperId: string, year: string): PaperData | null {
  const historyPaperTypeMapEng: Record<string, string> = {
    'p1': 'Paper 1',
    'p2': 'Paper 2',
  ...
  const historyPaperTypeMapChi: Record<string, string> = {
    'p1': '卷一',
    'p2': '卷二',
    'ans': '參考答案',
    'per': '考生表現報告'
  ...
  const displayType = language === 'chi' 
    ? historyPaperTypeMapChi[paperType] || paperType.toUpperCase()
    : historyPaperTypeMapEng[paperType] || paperType.toUpperCase();
...
export default function HistoryYearPage({ subject, year, papers, availableFiles }: YearPageProps) {
  // Use the clean single function approach
  const meta = generateYearMeta('history', year);
  const lastUpdated = getSubjectYearSlugLastUpdated('history');
  ...
  return (
    <>
      <Head>
        ...
        <meta property="og:url" content={`https://dse.best/history/${year}`} />
        ...
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              "url": `https://dse.best/history/${year}`,
              "mainEntity": {
      ...
      <div className="card rounded-4" style={{ height: "auto" }}>
        <div className="card-body">
          ...
          <div className="text-center mt-5 mb-5">
            <h3>Need More History Papers?</h3>
            ...
            >
              <span>View All History Papers (2012-2023)</span>
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  ...
  try {
    // Read history config file
    const configPath = path.join(process.cwd(), 'public', 'config', 'history.json');
    ...
    console.error(`Error loading history ${year}:`, error);
    return {
      notFound: true
    };
  }
