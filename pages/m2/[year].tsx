...
function getPaperDisplayInfo(paperId: string, year: string): PaperData | null {
  const m2PaperTypeMap: Record<string, string> = {
    'pp': 'Past Paper',
  ...
  const displayType = m2PaperTypeMap[paperType] || paperType.toUpperCase();
...
export default function M2YearPage({ subject, year, papers, availableFiles }: YearPageProps) {
  // Use the clean single function approach
  const meta = generateYearMeta('m2', year);
  const lastUpdated = getSubjectYearSlugLastUpdated('m2');
  ...
  return (
    <>
      <Head>
        ...
        <meta property="og:url" content={`https://dse.best/m2/${year}`} />
          ...
          {papers.length > 0 && (
            <div className="mb-5">
              <h3 className="text-center mb-4">
                <span style={{ color: '#0d6efd' }}>M2 Past Papers</span>
              </h3>
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  ...
  try {
    // Read m2 config file
    const configPath = path.join(process.cwd(), 'public', 'config', 'm2.json');
    ...
    console.error(`Error loading m2 ${year}:`, error);
    return {
      notFound: true
    };
  }
