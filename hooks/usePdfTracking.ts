...
/**
 * React hook to track PDF downloads
 * Replaces jQuery-based PDF tracking from main-tmp.js
 */
export const usePdfTracking = () => {
  ...
  useEffect(() => {
    ...
    const handlePdfClick = (event: MouseEvent) => {
      ...
      // Find the closest anchor element (handles clicks on child elements of links)
      const linkElement = target.closest('a');
      
      // Check if the clicked element is a PDF link
      if (linkElement && linkElement.href && linkElement.href.toLowerCase().endsWith('.pdf')) {
        ...
        if (extractedLanguage === 'unknown') {
          ...
          // Try to infer from page context, default to unknown
        ...
        const isYearSlug = Boolean(currentPath.match(/\/\d{4}$/));
        ...
        if (typeof window.gtag !== 'undefined') {
          ...
          window.gtag?.('event', 'pdf_download', simplifiedEventData);
        }
      }
    };

    // Add event listener for PDF clicks using event delegation
