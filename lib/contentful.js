...
const renderOptions = {
  renderNode: {
    ...
    [BLOCKS.EMBEDDED_ASSET]: (node) => {
      ...
      const width = f.file.details?.image?.width || 1000;
      const height = f.file.details?.image?.height || 600;
      return `<div class="my-4 text-center"><img src="${url}?fm=webp&w=1000" alt="${f.title || ''}" width="${width}" height="${height}" class="img-fluid" style="height: auto; max-width: 100%; display: inline-block;" /></div>`;
    },
...
function transformPost(post) {
  ...
  return {
    ...
    author: f.author || '',
    // Use system updatedAt for the sync logic comparison
    updatedAt: post.sys.updatedAt,
    // Keep the original custom date field for display if needed
    date: f.date || '',
